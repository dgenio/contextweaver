...
| **v0.18.0** | ✅ current (v0.18.0) | Compiler-first MVP (source snapshots, compiled-agent bundles, trust summaries, enrichment patches), mcp SDK safety pin, CI/CD hardening |
...
|---|---|---|---|---|---|
| **contextweaver** (this repo, [v0.18.0](https://pypi.org/project/contextweaver/0.18.0/)) | ✅ Bounded DAG + beam search · per-phase `ChoiceCard`s [^cw-route] | ✅ Phase-aware budgeted compilation · 43-85 % token reduction vs naïve [^cw-bench] | ✅ Built-in (size-gated, with `ArtifactRef` drilldown) [^cw-fire] | ✅ By default — tie-break by sorted IDs [^cw-det] | ✅ Native proxy + gateway runtimes per `docs/gateway_spec.md` [^cw-mcp] |
