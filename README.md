...
- Validated in CI on every PR against the published JSON Schemas at
...
| **I-05** — contextweaver receives Frames, not raw output | ✅ Satisfied via firewall | Tool outputs above `firewall_threshold` are stored out-of-band as `ArtifactRef`s; the LLM-facing surface is the `ResultEnvelope` (mapped to a spec `Frame` by [`adapters/weaver_contracts.py`](src/contextweaver/adapters/weaver_contracts.py)). |
...
validation against the published schemas at
`https://weaver-spec.dev/contracts/v0/`. Run locally with `make weaver-conformance`.
