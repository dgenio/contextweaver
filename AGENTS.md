...
| `store/_loop_thread.py` | The private `_LoopThread` (asyncio loop on a daemon thread) backing `_async_to_sync` (extracted to keep that module ≤300 lines). `run(coro, timeout=…)` bounds each op and raises `StoreTimeoutError` instead of hanging a stuck backend (issue #750). Not public API. |
...
| `__main__.py` | CLI: 16 top-level subcommands (`start`, `demo`, `build`, `route`, `print-tree`, `init`, `ingest`, `handoff`, `replay`, `stats`, `inspect`, `budget-check`, `eval`, `consolidate`, `verify`, `serve-api`) plus the `mcp` and `catalog` Typer sub-apps. `start` prints deterministic deployment-intent guidance without side effects (issue #660); `inspect` renders payload-safe context/routing/artifact reports (issue #398); `catalog lint` surfaces normalization + reference findings with `--json` and CI exit codes (issue #538). |
