...
- **Dependency-constraint policy** (issue #356). Specifiers are **lower-bound-only** (`>=`), set to the lowest version *actually known to work* — no `==` pins, no speculative upper caps. The only caps kept carry an inline rationale (pre-1.0 `weaver_contracts<1`; docs-extra major pins). Two CI jobs enforce this: a **gating floor-deps job** (`uv --resolution lowest-direct`, Python 3.10, in `ci.yml`) proves the `>=` floors, and a **non-gating weekly** `deps-latest-weekly.yml` (latest + pre-releases) is the no-upper-cap safety net. Raising a floor is a real change — verify with the floor-deps job. Python support is **3.10–3.14**, every cell gating in the CI matrix.
